...
export class NeedAssessmentRuleViewComponent implements OnInit {
  ...
  serviceData: any;
    ...
    add_route_link = '/need-assessment-rule/add';
   ...
   private loadServerData(dataset): void {
    ...
    button_click1(data) {
        ...
        this.router.navigateByUrl('need-assessment-rule/edit/' + data.OPTM_RULEID);
    ...
    duplicate_record(data){
        this.router.navigateByUrl('need-assessment-rule/add/' + data.OPTM_RULEID);
      }
        ...
        this.assessmentRuleService.DeleteData(this.GetItemData).subscribe(
            data => {
                ...
                if(data[0].IsDeleted == "0" && data[0].Message == "Rule Used"){
                    ...
                    this.service_call(this.current_page, this.search_string);
                    this.router.navigateByUrl('need-assessment-rule/view');
                }
