 ...
 // public isQuantityEnable = this.CommonService.quantityEnable;
  ...
  GetAllDataForSavedMultiModelBomOutput(data, saveddata) {
    ...
    if (data != null && data != undefined) {
      ...
      this.itemCodeList = data.ItemCodePartList;
      this.createItemCodeList();
      ...
      data => {
        ...
        if (data != undefined) {
          ...
          if (data.SelectedFeatureAttributes.length > 0) {
            if (data.SelectUsingAttribute != undefined) {
              this.filterList = data.SelectUsingAttribute;
            }
          }
  ...
  createItemCodeList() {
    ...
    let ItemCodeList = this.itemCodeList;
    this.featureItemList = [];

    defaultItemSelectList.forEach(element => {
      ItemCodeList.forEach(elementList => {
        if (element.OPTM_FEATUREID == elementList.OPTM_FEATUREID) {
          if (element.OPTM_LINENO == elementList.ITEMLINENO) {
            this.featureItemList.push({ "code": elementList.OPTM_ITEMCODE_PRT, "DESCORDER": elementList.DESCORDER })
          }
        }
      });
    });

    defaultAccesoryitemSelectList.forEach(element => {
      ItemCodeList.forEach(elementList => {
        if (element.OPTM_FEATUREID == elementList.OPTM_FEATUREID) {
          if (element.OPTM_LINENO == elementList.ITEMLINENO) {
            this.featureItemList.push({ "code": elementList.OPTM_ITEMCODE_PRT, "DESCORDER": elementList.DESCORDER })
    ...
    this.featureItemList.sort(function (a, b) {
      return a.DESCORDER - b.DESCORDER;
    });
  }
      ...
      for (var idtfeature in dtModelDataWithDefaultChild) {
        ...
        if (isExist.length == 0) {
          ...
          this.ModelBOMDataForSecondLevel.push({
            ...
            OPTM_DSPGROUP_ORDER: dtModelDataWithDefaultChild[idtfeature].OPTM_DSPGROUP_ORDER,
            OPTM_DSP_GROUP: dtModelDataWithDefaultChild[idtfeature].OPTM_DSP_GROUP,
            OPTM_DSP_ORDERINGROUP: dtModelDataWithDefaultChild[idtfeature].OPTM_DSP_ORDERINGROUP,
  ...
  onAddedModelChange(model_row_index, from_step4) {
    ...
    if (model_row_index != "" && model_row_index != undefined) {
      ...
      this.featureItemList = this.step2_selected_model.featureItemList;
      this.itemCodeList = this.step2_selected_model.itemCodeList;
      this.isModelVisible = true;
      this.feature_price_calculate();
      this.showLookupLoader = false;
      if (this.isAttribute) {
  ...
  fill_step3_data_array(mode, row_id) {
    ...
    if (mode == 'add') {
      ...
      this.step3_data_final[row_id]["featureItemList"] = this.featureItemList;
      this.step3_data_final[row_id]["itemCodeList"] = this.itemCodeList;
      ...
      for (let imandtory = 0; imandtory < isMandatoryItems.length; imandtory++) {
        ...
        if (isMandatoryItems[imandtory].OPTM_TYPE == "3") {
          ...
          if (isMandatoryItems[imandtory].parentmodelid != "" && isMandatoryItems[imandtory].parentmodelid != null) {
            ...
            for (let ifeatureBOMitems = 0; ifeatureBOMitems < this.FeatureBOMDataForSecondLevel.length; ifeatureBOMitems++) {
              if (isMandatoryItems[imandtory].unique_key == this.FeatureBOMDataForSecondLevel[ifeatureBOMitems].nodeid && this.FeatureBOMDataForSecondLevel[ifeatureBOMitems].checked == true) {
                get_min_select_list[imandtory]['actual']++;
              }
            }
          } else {
            ...
            for (let ifeatureBOMitems = 0; ifeatureBOMitems < this.FeatureBOMDataForSecondLevel.length; ifeatureBOMitems++) {
              if (this.FeatureBOMDataForSecondLevel[ifeatureBOMitems].checked == true && isMandatoryItems[imandtory].unique_key == this.FeatureBOMDataForSecondLevel[ifeatureBOMitems].nodeid) {
                get_min_select_list[imandtory]['actual']++;
              }
            }
          }
          ...
          for (let imodelBOMitems = 0; imodelBOMitems < this.ModelBOMDataForSecondLevel.length; imodelBOMitems++) {
            if (this.ModelBOMDataForSecondLevel[imodelBOMitems].checked == true && isMandatoryItems[imandtory].unique_key == this.ModelBOMDataForSecondLevel[imodelBOMitems].unique_key) {
              get_min_select_list[imandtory]['actual']++;
            }
          }
        }
    ...
    for (let iMinModelHeaderData = 0; iMinModelHeaderData < this.ModelHeaderData.length; iMinModelHeaderData++) {
      if (this.ModelHeaderData[iMinModelHeaderData].OPTM_MANDATORY == "Y") {
        if (this.ModelHeaderData[iMinModelHeaderData].OPTM_TYPE == "1") {
          var tempfeatureid = this.ModelHeaderData[iMinModelHeaderData].unique_key
          var isMinItemsCounted = this.FeatureBOMDataForSecondLevel.filter(function (obj) {
            return obj['nodeid'] == tempfeatureid && obj['checked'] == true
          })
    ...
    for (var imodelarray in ModelItemsData) {
      ...
      if (isExist.length == 0) {
        this.feature_itm_list_table.push({
          ...
          featureName: ModelItemsData[imodelarray].parent_code,
